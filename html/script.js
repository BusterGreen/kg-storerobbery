...
Padlock.Close = function () {
    ...
    $.post("https://kg-storerobbery/PadLockClose");
};
...
Keypad.Close = function (data) {
    ...
    $.post("https://kg-storerobbery/PadLockClose");
    if (data.error != null) {
        $.post("https://kg-storerobbery/CombinationFail");
    }
};
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post(
        "https://kg-storerobbery/TryCombination",
        JSON.stringify({
            combination: e.value,
        })
    );
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        if (!$(".num" + (i + 1)).hasClass("found")) {
            if (angle > (comboArr[i] - numOffset) * tickAngle && angle < (comboArr[i] + numOffset) * tickAngle) {
                ...
                // on unlock
                $.post("https://kg-storerobbery/callcops");
                if (i == comboArr.length - 1) {
                    // unlock :)
                    $.post("https://kg-storerobbery/PadLockSuccess");
                    Padlock.Close();
                }
            }
            break;
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) {
            if (CurrentType == "keypad") {
                ...
                $.post("https://kg-storerobbery/exit");
            }
        }
    };
}); //docready
...
function outOfPins() {
    gameOver = true;
    $.post("https://kg-storerobbery/fail");
    setTimeout(function () {
        reset();
    }, 250);
}
...
function unlock() {
    gameOver = true;
    $.post("https://kg-storerobbery/success");
    solveDeg = Math.random() * 180 - 90;
    solvePadding = 4;
    maxDistFromSolve = 45;
    pinRot = 1;
